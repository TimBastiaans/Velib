...
/* Created on:     1/9/2018 11:46:23 AM                         */
/*==============================================================*/
...
create table ABONNEMENTTYPE (
   ...
   LONGTERM2            WEL_OF_NIET          not null,
...
create table BESTELLING (
   ...
   DATUM_REPARATIE      DATUM                not null,
...
create table MONTEURDIENST (
   WERKNEMERID          ID_NUMMER            not null,
   START_DATUM2         DATUM                not null,
   WORKSHOPID           ID_NUMMER            not null,
   EIND_TIJD            DATUM                not null,
   constraint PK_MONTEURDIENST primary key nonclustered (WERKNEMERID, START_DATUM2)
)
go
...
create table REPARATIEQUEUE (
   ...
   FIETSID              ID_NUMMER            not null,
   BESCHRIJVING         BESCHRIJVING         null,
   FIETS_STATUS         BESCHRIJVING         null,
...
create table VOLTOOIDE_LENING (
   ...
   STATIONID            ID_NUMMER            not null,
   STA_STATIONID        ID_NUMMER            not null,
   EIND_TIJD            DATUM                not null,
   PRIJS                GELD                 not null,
   EXTRA_TIJD           WEL_OF_NIET          not null,
...
alter table UITGELEENDE_FIETS
   add constraint FK_UITGELEE_STARTSTAT_STATION foreign key (STATIONID)
      references STATION (STATIONID)
go

alter table VOLTOOIDE_LENING
   add constraint FK_VOLTOOID_BEGINSTAT_STATION foreign key (STATIONID)
      references STATION (STATIONID)
go

alter table VOLTOOIDE_LENING
   add constraint FK_VOLTOOID_EINDSTATI_STATION foreign key (STA_STATIONID)
      references STATION (STATIONID)
go
